...
> 日常启动唯一入口：普通会话只读本文件；`maintenanceDue` 到期才读 `workflows/context-maintenance.md`；大文件先搜索并按不超过 250 行分段读取。
...
## 计划自动触发门禁
执行任何实施任务前自动判定，无需用户额外提醒：项目级/分阶段开发、预计 3 个以上阶段、多个模块需分别验收，或计划超过 200 行时，必须读取 `workflows/project-planning.md`。若 `docs/plans/current.md` 已存在，则恢复当前阶段，不重新生成总计划；简单修改、单文件任务和普通 Bug 跳过。
